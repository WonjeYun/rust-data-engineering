...
fn main() {
    ...
    let mut names = vec!["ESPN".to_string(), "NFL".to_string(), "NBA".to_string(), "UFC".to_string(), "MLB".to_string()];
    ...
    println!("Do you want to add a website y/n?");
    loop {
        ...
        if answer == "y" {
            ...
            println!("Enter the indexes of the websites it links to or exit:");
            loop {
                let edge = read_input();
                if edge == "exit" {
                    break;
                }
                if let Ok(index) = edge.parse() {
                    edges.push(index);
                } else {
                    println!("Invalid input. Please enter a number or -1 to exit.");
                }
            }
            graph.push(edges);
        } else {
